...
MacOSX (macports) | port install py312-ansible
...
``` sudo port install py312-ansible ```<br>
``` ./mythtv.yml --limit=localhost ```
...
- Optionally specify a different version of python3
   ...
   ``` ./mythtv.yml --extra-vars="ansible_python_interpreter=/opt/local/bin/python3.12"  --limit=localhost ```
